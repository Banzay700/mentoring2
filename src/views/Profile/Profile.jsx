...
const ProfilePage = () => {
  ...
  const {
    ...
    error,
    isLoading,
  } = useGetProfileByIdQuery(id ?? localStorage.getItem("userId"));
      ...
      <Container maxWidth={"lg"}>
        ...
        >
          ...
          {isPersonalProfile && (
            ...
            >
              <MdPhotoCamera /> {!isMobile && "Change banner"}
            </WhiteButton>
          )}
        </Box>
        <Container sx={{ px: "5px" }}>
          ...
          >
            <AvatarMenu
              avatarUrl={profile?.avatarsUrl?.[0] ?? DEFAULT_USER_AVATAR}
            />
  ...
  return (
    <main>{error ?? isLoading ? <ProfileSkeleton /> : profileLayout}</main>
  );
};
